...
return [
    ...
    'validate' => [
        ...
        'handle' => shiyun\validate\handle\ThinkValidate::class,
        // 验证失败处理方法
...
> 注解类需要继承 `shiyun\annotation\AbstractAnnotation` 类
>
...
#### 方法一：通过 shiyun\annotation\AnnotationParse::yieldParseClassAnnotations() 方法获取
...
1. 新建Handle类
   注意：Handle类需要继承 `shiyun\annotation\IntfAnnotationHandle` 类
...
class TestHandle implements IntfAnnotationHandle
{
...
class CreateAnnotationHandle implements Bootstrap
{
    ...
    public static function start($worker)
    {
        ...
        // 添加测试控制器注解类处理器
        AnnotationParse::addHandle(TestControllerParams::class, TestHandle::class);
        // 添加测试控制器方法注解类处理器
        AnnotationParse::addHandle(TestMethodParams::class, TestHandle::class);
    }
}
```
